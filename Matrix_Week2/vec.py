...
def equal (u, v):
    ...
    assert u.D == v.D
    for element in u.D:
       if u[element] != v[element]: return False
    return True
...
def add(u, v):
    ...
    assert u.D == v.D
    return Vec(u.D,{k:v[k]+u[k] for k in u.D if v[k]+u[k]  != 0 })


def dot (u, v):
    "Returns the dot product of the two vectors"
    assert u.D == v.D
    return sum([u[item]*v[item] for item in u.D])


def scalar_mul (v, alpha):
    "Returns the scalar-vector product alpha times v"
    #for item in v.f.keys(): v[item]*=alpha
    #return v
    return Vec(v.D,{k:v[k]*alpha for k in v.D if v[k] != 0 })
...
class Vec:
    ...
    def __str__ (v):
        ...
        numdec=3
        wd=dict([
            (k, (1 + max(len(str(k)), len('{0:.{1}G}'.format(v[k], numdec))))) if isinstance(v[k], int) or isinstance(
